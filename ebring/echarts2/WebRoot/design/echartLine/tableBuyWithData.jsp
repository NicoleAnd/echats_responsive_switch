...
<script type="text/javascript">
	...
	$('#table').on('click-row.bs.table', function (e, row, element){
		window.parent.document.getElementById("custNo").value = row.custNo;
		window.parent.seleCustData();
	});
	$('#table').bootstrapTable({
		...
		columns : [ {
			...
			formatter : function(value,row,index){
				if(value != null && value != ''){
					var pivalue = (parseInt((parseFloat(value)*10000)/1))/100;
					var str ="<span>"+pivalue+"<span>%</span>"+"</span>";
					return str;
				}
				return value;
			}
